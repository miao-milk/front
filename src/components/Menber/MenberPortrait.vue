<template>
  <div id="menberl-portrait">
    <el-row :gutter="20">
      <el-col :span="8">
        <div id="main" style="width: 600px; height: 500px"></div
      ></el-col>
      <el-col :span="12" :offset="4">
        <el-card class="box-card">
          <div slot="header" class="clearfix">
            <span>用户个性名片</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="handleClick()">立即营销</el-button
            >
          </div>
          <!-- <div v-for="o in 4" :key="o" class="text item">
        {{'列表内容 ' + o }}
    </div> -->
          <div class="text item">买家昵称:{{ people.buyerNick }}</div>
          <div class="text item">姓名:{{ people.receiverName }}</div>
          <div class="text item">手机号码：{{ people.receiverMobile }}</div>
          <div class="text item">
            性别：
            <span v-if="people.sex === 1">男</span>
            <span v-if="people.sex === 0">女</span>
          </div>
          <div class="text item">出生年份：{{ people.bornYear }}</div>
          <div class="text item">星座：{{ people.constellation }}</div>
          <div class="text item">微信：{{ people.weChat }}</div>
          <div class="text item">qq：{{ people.qq }}</div>
          <div class="text item">联系地址：{{ people.receiverAddress }}</div>
          <div class="text item">
            会员等级：
            <span v-if="people.grade === 0">新会员</span>
            <span v-if="people.grade === 1">普通会员</span>
            <span v-if="people.grade === 2">高级会员</span>
            <span v-if="people.grade === 3">尊贵会员</span>
          </div>
          <div class="text item">交易次数：{{ people.buyTotalCount }}</div>
          <div class="text item">交易金额：{{ people.buyTotalMoney }}</div>
          <div class="text item">付款件数：{{ people.paymentNum }}</div>
          <div class="text item">均件数：{{ people.avgNum }}</div>
          <div class="text item">最后下单时间：{{ people.buyerLastTime }}</div>
        </el-card></el-col
      >
    </el-row>
  </div>
</template>

<script>
//需要将安装的插件进行引入
import "echarts-wordcloud";
import { getMemberDetail,getMemberLabelBySellerId } from "../../api";
export default {
  data() {
    return {
      people: {},
      jsonlist:[
        {
          name: "缪孝文",
          value:200
        }
      ]
    };
  },
  methods:{
    async chartPortrait(){
     const chart = this.$echarts.init(document.getElementById("main"));
    //用来存储数据
    var shopBuyerId = this.$route.query.shopBuyerId;
    //调用请求
    await getMemberLabelBySellerId(shopBuyerId).then((data) => {
       data.data.forEach(value => {
         this.jsonlist.push({
         name: value.name,
         value:value.value
         })
      })
    })
     // 人像的base64编码
    var image1 =
      "data:image/png;base64,";

    var maskResource = new Image();
    maskResource.src = image1;
    var option = {
      //设置标题，居中显示
      title: {
        text: "用户画像",
        left: "center",
      },
      //数据可以点击
      tooltip: {},

      series: [
        {
          maskImage: maskResource,
          //词的类型
          type: "wordCloud",
          //设置字符大小范围
          sizeRange: [6, 78],
          rotationRange: [-45, 90],
          textStyle: {
            normal: {
              //生成随机的字体颜色
              color: function () {
                return (
                  "rgb(" +
                  [
                    Math.round(Math.random() * 160),
                    Math.round(Math.random() * 160),
                    Math.round(Math.random() * 160),
                  ].join(",") +
                  ")"
                );
              },
            },
          },
          //不要忘记调用数据
          data: this.jsonlist,
        },
      ],
    };
    //加载图像，将数据放在图像中
    maskResource.onload = function () {
      chart.setOption(option);
    };
    },
    handleClick() {
        this.$router.push({name:'Message',query:{sellerId:this.people.sellerId}})
    },
  },
  mounted() {
    console.log(this.$route.query);
    var shopBuyerId = this.$route.query.shopBuyerId;
    getMemberDetail(shopBuyerId).then((data) => {
      console.log(data);
      this.people = data.data;
    });
    this.chartPortrait();
  },
};
</script>

<style>
.text {
  font-size: 14px;
}

.item {
  margin-bottom: 18px;
}

.clearfix:before,
.clearfix:after {
  display: table;
  content: "";
}
.clearfix:after {
  clear: both;
}

.box-card {
  width: 680px;
}
</style>